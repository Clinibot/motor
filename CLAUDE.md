...
- **`undefined` vs `null` en PATCH a Retell (crítico)**: el SDK de Retell omite los campos con valor `undefined` del cuerpo del PATCH — Retell interpreta esto como "no cambies este campo" y mantiene el valor anterior. Para **limpiar** un campo hay que enviar `null` explícitamente. Afecta a: `ambient_sound` (acepta `null`), `fallback_voice_ids` (se envía `[]`), y cualquier otro campo opcional que el usuario pueda desactivar. Excepción: `ambient_sound_volume` usa `undefined` porque el SDK de Retell solo acepta `number | undefined` — cuando `ambient_sound` es `null`, Retell descarta el volumen igualmente. Síntoma típico: el usuario desactiva una opción en el wizard, guarda, pero en Retell el valor antiguo persiste.

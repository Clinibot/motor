...
export default function HelpPage() {
  ...
  useEffect(() => {
    ...
    const helpArticles: Record<string, { cat: string; title: string; updated: string; body: string }> = {
      ...
      'wizard-llm': {
        cat: 'Configurar el agente', title: 'Paso 2 — Modelo de IA y comportamiento', updated: '18 mar 2026',
        body: `<p>En este paso configuras el cerebro de tu agente: qué modelo de IA usa, cómo arranca las conversaciones y qué personalidad tiene.</p><h2>Modelo de IA</h2><ul><li><strong>Gemini 3.0 Flash</strong> — Recomendado. Mejor equilibrio entre calidad y velocidad.</li><li><strong>GPT-4.1</strong> — Alta capacidad de razonamiento. Ideal para conversaciones complejas.</li></ul><h2>Mensaje de inicio</h2><p>Es lo primero que dice el agente. Debe incluir presentación, aviso de IA (obligatorio) y aviso de grabación (obligatorio por LOPD).</p><div class="help-callout help-callout-warn"><span>⚠</span><span><strong>Obligatorio por ley española:</strong> El mensaje de inicio debe incluir que es un asistente de voz con IA y que la llamada está siendo grabada. No elimines estas declaraciones.</span></div>`,
      },
      'wizard-voz': {
    ...
    const botMap: Record<string, { t: string; a: string | null }> = {
      ...
      'modelo': { t: 'Puedes elegir entre <strong>Gemini 3.0 Flash</strong> (recomendado) y <strong>GPT-4.1</strong> (más potente). Se configura en el paso 2.', a: 'wizard-llm' },
      'gemini': { t: 'Gemini 3.0 Flash es el modelo recomendado. Ofrece el mejor equilibrio entre calidad y velocidad de respuesta.', a: 'wizard-llm' },
      'gpt': { t: 'GPT-4.1 tiene alta capacidad de razonamiento, pero es ligeramente más lento que Gemini. Ideal para conversaciones complejas.', a: 'wizard-llm' },

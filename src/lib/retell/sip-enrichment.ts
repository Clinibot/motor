...
export async function enrichSipCredentials(payload: any, supabaseAdmin: SupabaseClient, agentId?: string) {
    ...
    if (agentId) {
        console.log(`Searching for SIP credentials associated with agent ${agentId}...`);
        const { data: assignedNumber } = await supabaseAdmin
            .from('phone_numbers')
        ...
        if (assignedNumber) {
            console.log(`Found assigned number ${assignedNumber.phone_number} for agent. Using its SIP credentials.`);
    ...
    for (const dest of payload.transferDestinations) {
        if (dest.destination_type === 'number' && dest.number) {
            ...
            if (workspaceSipUser && workspaceSipPass) {
                console.log(`Enriching transfer to ${dest.number} with agent's SIP credentials.`);
                dest.sip_username = workspaceSipUser;
                dest.sip_password = workspaceSipPass;
                continue;
            }

            // Fallback: Buscar credenciales del número de destino (por si es una transferencia entre internos de la misma plataforma)
